...
public class HTEopdSensor extends Sensor
{
  ...
  protected void init()
  {
    if (LegoRobot.getDebugLevel() >= SharedConstants.DEBUG_LEVEL_MEDIUM)
      DebugConsole.show("DEBUG: HTEopdSensor.init() called (Port: "
        + getPortLabel() + ")");
  }
  ...
  {
    if (LegoRobot.getDebugLevel() >= SharedConstants.DEBUG_LEVEL_MEDIUM)
      DebugConsole.show("DEBUG: HTEopdSensor.cleanup() called (Port: "
        + getPortLabel() + ")");
  }
  ...
  {
    if (robot == null)
      new ShowError("HTEopdSensor (port: " + getPortLabel()
