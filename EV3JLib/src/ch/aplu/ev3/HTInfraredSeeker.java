...
public class HTInfraredSeeker extends Sensor
{
  ...
  protected void init()
  {
    if (LegoRobot.getDebugLevel() >= SharedConstants.DEBUG_LEVEL_MEDIUM)
      DebugConsole.show("DEBUG: HTInfraredSeeker.init() called (Port: "
        + getPortLabel() + ")");
  }
  ...
  {
    if (LegoRobot.getDebugLevel() >= SharedConstants.DEBUG_LEVEL_MEDIUM)
      DebugConsole.show("DEBUG: HTInfraredSeeker.cleanup() called (Port: "
        + getPortLabel() + ")");
  }
  ...
  {
    if (robot == null)
      new ShowError("HTInfraredSeeker (port: " + getPortLabel()
