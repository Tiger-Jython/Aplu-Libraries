...
//        System.out.println("method not accessible");
    ...
    public void act()
    {
      synchronized (MbRobot.class)
      {
        String info = RobotContext.statusText;
        if (!info.equals(MbRobot.statusText))
        {
          MbRobot.statusText = info;
        ...
        if (!motors.isEmpty() && motors.size() == 2)
        {
          ...
          {
            MbRobot.debug("Motors state change");
          ...
          {
            MbRobot.debug("motorA FORWARD; motorB FORWARD");
            if (speedA == speedB)
            {
          ...
          {
            MbRobot.debug("motorA BACKWARD; motorB BACKWARD");
            if (speedA == speedB)
            {
              advance(-SharedConstants.nbSteps * speedA);
              mark.setLocation(new Location(-100, -100));
          ...
          {
            MbRobot.debug("motorA BACKWARD; motorB FORWARD");
            if (speedA == speedB)
            {
          ...
          {
            MbRobot.debug("motorA FORWARD; motorB BACKWARD");
            if (speedA == speedB)
            {
              turn((int)(speedA * SharedConstants.motTurnAngle));
              if (RobotContext.isRotCenterEnabled)
          ...
          {
            MbRobot.debug("motorA STOPPED; motorB FORWARD");
            if (isRotationInit)
            {
              isRotationInit = false;
          ...
          {
            MbRobot.debug("motorA STOPPED; motorB.BACKWARD");
            if (isRotationInit)
            {
              isRotationInit = false;
              radius = wheelDistance / 2;
          ...
          {
            MbRobot.debug("motorA FORWARD; motorB STOPPED");
            if (isRotationInit)
            {
              isRotationInit = false;
              radius = wheelDistance / 2;
          ...
          {
            MbRobot.debug("motorA BACKWARD; motorB STOPPED");
            if (isRotationInit)
            {
              isRotationInit = false;
              radius = wheelDistance / 2;
          ...
          {
            MbRobot.debug("motorA STOPPED; motorB STOPPED");
            return;
          }
        }
      }
