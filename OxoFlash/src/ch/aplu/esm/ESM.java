// ESM.java
// Duplication for OxoFlash, only cmd-params changed (see ctor)
...
public class ESM implements SerialPortEventListener
{
  ...
  private final String VERSION = "1.01 - June 8, 2018";
  // ------------------------------------------------------
  ...
  private final String versionInfo
    = "OxoFlash V" + VERSION + " [www.aplu.ch]";
  private final String noCommPort = "COM port no found. Possible reasons:\n"
    + "   Not connected -> Plug module into USB port\n"
    + "   Automatic COM port search failed or wrong COM port -> Use -p option or select COM port in TigerJython Preferences Menu (Library tag)\n"
  ...
  // ------------------- Constructor -----------------------
  public ESM(String[] argsx)
  {
    String[] args =
    {
      "-f", "-m"
    };
    ch.aplu.util.Console c = ch.aplu.util.Console.init();
  ...
  private boolean finish()
  {
    ...
    byte[] bAry = new byte[]
    {
      (byte)0x03, (byte)0x03  // 2x ^C
    };
    SendThread t = new SendThread(bAry);  // ctrl+C->stop
    t.start();
    try
    {
      t.join(5000);
...
//            System.out.println(ch);
  ...
  private boolean flash()
  {
    ...
    if (oxocard)
    {
      ...
      System.out.println("ESP32 selected.");
    ...
    if (blockly)
    {
      ...
      flashData = firmwarePath + "phy_init_data_blockly.bin";
