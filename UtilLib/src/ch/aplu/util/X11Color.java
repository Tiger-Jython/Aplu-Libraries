...
public class X11Color extends java.awt.Color
{
   ...
   * @return the X11 color name or the hex value "#rrggbb, if the given color is not part of
   * the implemented X11 colors (lower case letters)
   */
  public static String toColorStr(Color color)
  {
    for (String key : table.keySet())
    {
      ...
      if (c >= '0' && c <= '9')  // Skip names with ending number
        continue;
      if (key.contains(" ")) // Skip names space
        continue;
      if (key.contains("grey")) // Skip names with British gray
        continue;
