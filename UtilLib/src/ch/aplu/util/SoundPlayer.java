...
public class SoundPlayer
{
  ...
  {
    File file = new File(audioPathname);
    String absPath = file.getAbsolutePath();
    if (new File(absPath).exists())
    {
      this.audioPathname = absPath;
...
//    currentPos = currentTime * frameRate
   ...
   * Use URLfromJAR(String audioPath) to load from JAR resource.
   */
   ...
   * Audio format: 8 bit signed, mono, 22050 Hz, static method.
   ...
   */
  public static void playSine(double frequency, double duration)
  {
    playSine(100, frequency, duration, true);
  }
   ...
   */
  public static void playSine(double volume, double frequency, double duration, boolean blocking)
  {
    ...
    int amplitude = Math.min(100, (int)(volume));
    ...
    AudioInputStream audioInputStream
      = new AudioInputStream(is, myAudioFormat,
        data.size() / myAudioFormat.getFrameSize());
    SoundPlayer player = null;
    try
    {
      player = new SoundPlayer(audioInputStream);
      if (blocking)
      {
