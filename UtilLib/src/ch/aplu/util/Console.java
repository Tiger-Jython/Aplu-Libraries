...
// ------------------------ Internal classes -------------------
  ...
  {
    private boolean inhibitRepeat = false;

    public void keyPressed(KeyEvent evt)
    {
      if (inhibitRepeat) // Error deleting char when too fast
        return;
      _gotKey = true;
    ...
    private String getLastLine()
    {
      try
      {
        int end = _textArea.getDocument().getLength();
        int start = Utilities.getRowStart(_textArea, end);

        while (start == end)
        {
          end--;
          start = Utilities.getRowStart(_textArea, end);
        }

        return _textArea.getText(start, end - start);
...
// 
  ...
  private void createConsole(Position position, Size size, Font font)
  {
    ...
    Action beep = _textArea.getActionMap().get(DefaultEditorKit.deletePrevCharAction);
...
//      _textArea.insert(msg, _charCount);
   ...
   * (No echo in the console window.)
   */
  public static char getKey(boolean keep)
  {
  ...
  public static void deleteLastChar()
  {
    ...
    _charCount -= 1;
    String text = _textArea.getText().substring(0, _charCount);
    if (SwingUtilities.isEventDispatchThread())
    {
  ...
  /**
   * Initialize all static variables (for Jython users).
   */
  public static void initStatics()
  {
    ...
    _quit = false;
    _gotKey = false;
    _keyChar = KeyEvent.CHAR_UNDEFINED;
    _keyCode = 0;
    _modifiers = 0;
    _modifiersText = "";
    _charCount = 0;
