...
//    System.out.println("getExtractedImage(" + x1 + "," + y1 + "), (" + x2 + "," + y2 + ")");
   ...
   * from a internet server. Supported formats are jpg, bmp, png, gif.<br><br>
  ...
  /**
   * Retrieve the image from a byte array. The supported
   * formats are jpg, bmp, png, gif.
   * @param ary the data to read
   * @return the buffered image or null, if the image cannot be converted
   */
  public static synchronized GBitmap readImage(byte[] ary)
  {
    BufferedImage bi = null;
    GBitmap dest = null;
    try
    {
      InputStream is = new ByteArrayInputStream(ary);
      bi = ImageIO.read(is);
      dest = new GBitmap(bi.getWidth(), bi.getHeight(),
        BufferedImage.TYPE_INT_ARGB);
      Graphics2D g2D = dest.createGraphics();
      g2D.drawImage(bi, 0, 0, null);
      g2D.dispose();
