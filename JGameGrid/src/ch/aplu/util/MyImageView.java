...
class MyImageView extends View implements ImageObserver, MouseListener, MouseMotionListener
{
  ...
  private void initialize(Element elem)
  {
    ...
    try
    {
      ...
      if (HtmlPane.debug)
        System.out.println("MyImageView: initializing...");
      ...
      else if (isOnline())
      {
        ...
        if (HtmlPane.debug)
          System.out.println("MyImageView: got homeUrl = " + homeUrl);
        String imageUrl = homeUrl + "/" + tag;
        if (HtmlPane.debug)
          System.out.println("MyImageView: imageUrl = " + imageUrl);
        URL src = getSourceURL();
        try
        {
      ...
      {
        /******** Code to load local file from relative path *************/
        String src = (String)fElement.getAttributes().getAttribute(HTML.Attribute.SRC);
        if (HtmlPane.debug)
          System.out.println("MyImageView: loading image with relative path = " + src);
  ...
  private boolean isURLPrefixed()
  {
    String src = (String)fElement.getAttributes().getAttribute(HTML.Attribute.SRC);
    if (src.toLowerCase().startsWith("file://"))
      System.out.println("file:// prefix not supported for loading images");
    if (HtmlPane.debug)
      System.out.print("Calling MyImageView.isUrlPrefixed() with src = " + src);
    boolean isPrefixed = src.toLowerCase().startsWith("http://");
    if (HtmlPane.debug)
      System.out.println("...returning " + isPrefixed);
    return isPrefixed;
  }

  private boolean isOnline()
  {
    boolean online = (homeUrl != null);
    if (HtmlPane.debug)
      System.out.println("MyImageView.isOnline = " + online);
    return online;
  }
