...
/**
 * Class that represents a simulated MbRobot.
...
public class Callibot
{
    ...
    public boolean keyPressed(KeyEvent e)
    {
      ...
      switch (e.getKeyCode())
      {
        ...
        case 40:
          ...
          break;
        case 65:
          buttonID = BrickButton.ID_A;
          break;
        case 66:
          buttonID = BrickButton.ID_B;
          break;
      }
    ...
    implements GGActorCollisionListener, GGExitListener
  {
    ...
    {
      super(true, "sprites/mbrobot.gif");  // Rotatable
...
//        System.out.println("method not accessible");
    ...
    public void act()
    {
      synchronized (Callibot.class)
      {
        String info = RobotContext.statusText;
        if (!info.equals(Callibot.statusText))
        {
          Callibot.statusText = info;
        ...
        if (!motors.isEmpty() && motors.size() == 2)
        {
          ...
          {
            Callibot.debug("Motors state change");
          ...
          {
            Callibot.debug("motorA FORWARD; motorB FORWARD");
            if (speedA == speedB)
            {
          ...
          {
            Callibot.debug("motorA BACKWARD; motorB BACKWARD");
            if (speedA == speedB)
            {
              advance(-SharedConstants.nbSteps * speedA);
              mark.setLocation(new Location(-100, -100));
          ...
          {
            Callibot.debug("motorA BACKWARD; motorB FORWARD");
            if (speedA == speedB)
            {
          ...
          {
            Callibot.debug("motorA FORWARD; motorB BACKWARD");
            if (speedA == speedB)
            {
              turn((int)(speedA * SharedConstants.motTurnAngle));
              if (RobotContext.isRotCenterEnabled)
          ...
          {
            Callibot.debug("motorA STOPPED; motorB FORWARD");
            if (isRotationInit)
            {
              isRotationInit = false;
          ...
          {
            Callibot.debug("motorA STOPPED; motorB.BACKWARD");
            if (isRotationInit)
            {
              isRotationInit = false;
              radius = wheelDistance / 2;
          ...
          {
            Callibot.debug("motorA FORWARD; motorB STOPPED");
            if (isRotationInit)
            {
              isRotationInit = false;
              radius = wheelDistance / 2;
          ...
          {
            Callibot.debug("motorA BACKWARD; motorB STOPPED");
            if (isRotationInit)
            {
              isRotationInit = false;
              radius = wheelDistance / 2;
          ...
          {
            Callibot.debug("motorA STOPPED; motorB STOPPED");
            return;
          }
        }
      }
  ...
  private String title = "MbRobotSim V"
  ...
  /**
   * Returns true, if the A button was the last button hit since
   * the last call of this method. On return, the button hit is cleared.
   * Calls Thread.sleep(10) to be used in narrow loops.
   * @return true, if the up button was clicked or the simulation window is closed
   */
  public boolean is_A_hit()
  {
    Tools.delay(10);
    if (gg.isDisposed())
      return true;
    boolean pressed = (buttonID == BrickButton.ID_A);
    if (pressed)
      buttonID = 0;
    return pressed;
  }
  ...
  /**
   * Returns true, if the B button was the last button hit since
   * the last call of this method. On return, the button hit is cleared.
   * Calls Thread.sleep(10) to be used in narrow loops.
   * @return true, if the up button was clicked or the simulation window is closed
   */
  public boolean is_B_hit()
  {
    Tools.delay(10);
    if (gg.isDisposed())
      return true;
    boolean pressed = (buttonID == BrickButton.ID_B);
    if (pressed)
      buttonID = 0;
    return pressed;
  }
...
//   float sampleRate = 44100.0F;
  ...
  private void initWheelActors()
  {
    if (leftWheel == null || rightWheel == null)
    {
      ...
      leftWheel = new Actor(true, "sprites/leftwheel1.gif");
      rightWheel = new Actor(true, "sprites/rightwheel1.gif");
